...
public class Excel2Pdf extends PdfTool{
    ...
    public void convert() throws DocumentException, IOException {
        ...
        if(this.objects.size() <= 1){
            PdfPTable table = this.toCreatePdfTable(this.objects.get(0));
        ...
        if(this.objects.size() > 1){
            ...
            for (int i = 0; i < this.objects.size(); i++) {
                PdfPTable table = this.toCreatePdfTable(this.objects.get(i));
                getDocument().add(table);
            }
        }
        //
...
//      table.setWidthPercentage(new float[]{100} , writer.getPageSize());
    ...
    private static class PDFPageEvent extends PdfPageEventHelper {
        ...
        public void onStartPage(PdfWriter writer, Document document) {
            try{
                this.template = writer.getDirectContent().createTemplate(80, 80);
